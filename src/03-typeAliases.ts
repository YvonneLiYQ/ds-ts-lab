import {
  ...
  function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce((acc: Friend[], buddy: Buddy) => {

      if ('age' in buddy) {
        acc.push(buddy as Friend); 
      }
      return acc;
    }, []);
  }
  
  const friendsFromBuddyList = getBuddyListFriends(myBandBuddies);
  console.log(friendsFromBuddyList);
